...
@WebSocketGateway({
	cors: { 
		origin: process.env.FRONTEND_URLS?.split(',').map(url => url.trim()).filter(Boolean).concat([
			'http://localhost:3000',
			'http://localhost:3002',
...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async handleMessageSend(client: Socket, payload: { channelId: string; content: string }) {
		if (!client.data.userId) {
			client.emit('error', { message: 'Not authenticated' })
			return
		}
		...
		try {
			...
			const message = await this.chatService.sendMessage(payload.channelId, user.id, payload.content)
